...
export async function up(knex: Knex) {
  const insertedHeader = await knex.raw(`
    INSERT INTO invasivesbc.code_header (code_category_id, code_header_name, code_header_title, code_header_description,
                                         ...
                                         updated_by_user_id)
    VALUES (2, 'previous_inspection_days', 'previous_inspection_days',
            'Previous Inspection and/or Agency Notification Number of Days', '2023-04-21 04:36:45.63048+08', NULL,
  ...
  await knex.raw(`
    INSERT INTO invasivesbc.code(code_header_id, code_name, code_description, code_sort_order, created_by_user_id,
                                 updated_by_user_id)
    VALUES (${codeHeaderId}, 'PID-1', '< 30 Days', 1, 1, 1);
    INSERT INTO invasivesbc.code(code_header_id, code_name, code_description, code_sort_order, created_by_user_id,
                                 updated_by_user_id)
    VALUES (${codeHeaderId}, 'PID-2', '> 1 Year', 2, 1, 1);
    INSERT INTO invasivesbc.code(code_header_id, code_name, code_description, code_sort_order, created_by_user_id,
                                 updated_by_user_id)
    VALUES (${codeHeaderId}, 'PID-3', '> 30 Days', 3, 1, 1);
    INSERT INTO invasivesbc.code(code_header_id, code_name, code_description, code_sort_order, created_by_user_id,
                                 updated_by_user_id)
    VALUES (${codeHeaderId}, 'PID-4', 'Same Day', 4, 1, 1);`);
}
...
export async function down(knex: Knex) {
  const insertedHeader = await knex.raw(`
    ...
    WHERE code_header_name = 'previous_inspection_days';
  `);
