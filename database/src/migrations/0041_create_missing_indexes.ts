...
export async function up(knex: Knex): Promise<void> {
  await knex.raw(
    ...
    CREATE index concurrently if not exists iapp_site_summary_and_geojson_site_id_idx ON invasivesbc.iapp_site_summary_and_geojson 
    ...
    CREATE index concurrently if not exists point_of_interest_biological_dispersal_date_idx 
    ...
    CREATE index concurrently if not exists point_of_interest_biological_treatment_date_idx 
    ON invasivesbc.point_of_interest_incoming_data 
    ...
    CREATE index concurrently if not exists point_of_interest_biological_treatment_monitoring_date_idx 
    ON invasivesbc.point_of_interest_incoming_data 
    ...
    CREATE index concurrently if not exists point_of_interest_chemical_monitoring_date_idx 
    ON invasivesbc.point_of_interest_incoming_data 
    ...
    CREATE index concurrently if not exists point_of_interest_chemical_treatment_date_idx 
    ON invasivesbc.point_of_interest_incoming_data 
    ...
    CREATE index concurrently if not exists point_of_interest_id_idx 
    ON invasivesbc.point_of_interest_incoming_data 
    USING btree (point_of_interest_id);
    
    CREATE index concurrently if not exists point_of_interest_mechanical_treatment_date_idx 
    ON invasivesbc.point_of_interest_incoming_data 
    ...
    CREATE index concurrently if not exists point_of_interest_mechanical_treatment_monitoring_date_idx 
    ON invasivesbc.point_of_interest_incoming_data 
    ...
    CREATE index concurrently if not exists point_of_interest_survey_date_idx 
    ON invasivesbc.point_of_interest_incoming_data 
    USING btree (invasivesbc.immutable_to_date(((((point_of_interest_payload -> 'form_data'::text) -> 'surveys'::text) -> 'survey_date'::text))::text));
    
    CREATE index concurrently if not exists admin_defined_shapes_geog_idx 
