...
export async function up(knex: Knex): Promise<void> {
  await knex.raw(
    ...
    VALUES(44, 'VAA', 'Victoria Airport Authority', 1, now(), null, now(), now(), 1, 1),
    (79, 'VAA', 'Victoria Airport Authority', 1, now(), null, now(), now(), 1, 1),
    (79, 'SHCL', 'Shearing Consultants Limited', 1, now(), null, now(), now(), 1, 1),
    (79, 'CCD', 'Cariboo Chilcotin Divers Limited', 1, now(), null, now(), now(), 1, 1);
...
export async function down(knex: Knex): Promise<void> {
  ...
  AND code_name IN ('VAA','SHCL','CCD'))
  OR (code_header_id = 44 and code_name in ('VAA'));
