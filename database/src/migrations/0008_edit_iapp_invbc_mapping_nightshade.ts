...
export async function up(knex: Knex) {
  ...
  await knex.raw(
    ...
    SELECT 'NE', 'Silverleaf nightshade (Solanum elaeagnifolium)', '', 'T', '', 'Silverleaf nightshade (Solanum elaeagnifolium)'
    WHERE NOT EXISTS (
      SELECT 1 FROM invasivesbc.iapp_invbc_mapping WHERE char_code = 'NE'
    );
