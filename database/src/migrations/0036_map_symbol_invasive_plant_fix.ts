...
export async function up(knex: Knex) {
  await knex.raw(
    ...
    `
      ...
      begin
        return QUERY
          with user_details as (select idir_account_name,
                                       bceid_account_name,
                                       funding_agencies,
                                       employer
...
-- drop generated always as ...
      alter table invasivesbc.activity_incoming_data
        alter column invasive_plant drop expression if exists;
...
-- add index
      ...
      begin
        ...
        with invasive_plants as (select a.activity_incoming_data_id,
                                        string_agg(distinct c.code_description, ', '
                                                   order by c.code_description) as invasive_plant
        ...
        set invasive_plant = ip.invasive_plant
        from invasive_plants ip
        where aid.activity_incoming_data_id = ip.activity_incoming_data_id
      ...
      -- add trigger
      create trigger invasive_plant
        after
          insert
        on
          invasivesbc.activity_incoming_data
        for each row
      execute function invasivesbc.update_invasive_plant();
