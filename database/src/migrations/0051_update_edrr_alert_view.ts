...
export async function up(knex: Knex): Promise<void> {
  await knex.raw(
    ...
    public;
  drop view if exists edrr_species;
  create 
  or replace view edrr_species as (
    with species_select as (
      ...
      from 
        ...
        left join application_user a on a.preferred_username = created_by 
      where 
        ...
        and form_status = 'Submitted' and received_timestamp >= now() + interval '6 hours'
        ), 
    species_array as (
      select 
        activity_incoming_data_id, 
