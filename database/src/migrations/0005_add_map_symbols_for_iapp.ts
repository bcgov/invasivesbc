...
export async function up(knex: Knex) {
  ...
  await knex.raw(
    ...
    delete from iapp_invbc_mapping 
    where mapping_id = 45;
    ...
    where code_header_id = 27 and code_name in ('L', 'M', 'H', 'D');
    ...
    where code_header_id = 29 and code_name in ('RS', 'FS', 'CL', 'SS', 'FP', 'WS', 'CU', 'CO', 'CD');
    
    update code 
    set code_description = 'Reed canary grass (Phalaris arundinacea)'
    where code_header_id = 39 and code_name = 'RE';
    
    update code 
    set code_description = 'Giant mannagrass / reed sweetgrass (Glyceria maxima)'
    where code_header_id = 39 and code_name = 'SW';
    ...
    where iapp_name in ('Didymo (DIDY GEM)', 'Knapweed species (CENT SPP)', 'Sowthistle species (SONC SPP)', 'Yellow hawkweed (HIER PRA)');
