...
export async function up(knex: Knex) {
  ...
  await knex.raw(`
    ...
    SELECT role_description,
           role_name,
           metabase_group
    FROM (VALUES ('Primary Biocontrol User', 'biocontrol_user',
  ...
  await knex.raw(`
    ...
    SELECT code_header_id,
           ...
           created_by_user_id,
           updated_by_user_id
    FROM (VALUES (43, 'SCLESCL', 'SCLESCL [Sclerotinia sclerotium]', 1, 1),
                 ...
                 (43, 'BANGFAU', 'BANGFAU [Bangasternus fausti]', 1, 1)) AS new_codes (code_header_id, code_name,
                                                                                       code_description,
                                                                                       created_by_user_id,
                                                                                       updated_by_user_id)
    WHERE NOT EXISTS (SELECT 1
  ...
  // Update Sort Order
  await knex.raw(`
    WITH ordered_alphabetically AS (SELECT code_id,
  ...
  await knex.raw(`
    ...
    INSERT
    INTO invasivesbc.private_biocontrol_agents(agent_code_pkey, agent_code_name, agent_code_description)
  ...
  await knex.raw(`
    WITH admins AS (SELECT user_id
                    FROM invasivesbc.user_access
                    WHERE role_id = 18),
         biocontrol_role AS (SELECT role_id
                             FROM invasivesbc.user_role
                             WHERE role_name = 'biocontrol_user')
    INSERT
    INTO invasivesbc.user_access (user_id, role_id)
  ...
  await knex.raw(`
    WITH aafc_users AS (SELECT user_id
...
export async function down(knex: Knex) {
  ...
  await knex.raw(`
    DELETE
    FROM invasivesbc.code
    WHERE code_name = 'SCLESCL'
      and code_header_id = 43;
    DELETE
    FROM invasivesbc.code
    WHERE code_name = 'CEUTSCR'
      and code_header_id = 43;
    DELETE
    FROM invasivesbc.code
    WHERE code_name = 'ACERANG'
      and code_header_id = 43;
    DELETE
    FROM invasivesbc.code
    WHERE code_name = 'DICHAER'
      and code_header_id = 43;
    DELETE
    FROM invasivesbc.code
    WHERE code_name = 'ARCHNEU'
      and code_header_id = 43;
    DELETE
    FROM invasivesbc.code
    WHERE code_name = 'BAGONOD'
      and code_header_id = 43;
    DELETE
    FROM invasivesbc.code
    WHERE code_name = 'LENIGEM'
      and code_header_id = 43;
    DELETE
    FROM invasivesbc.code
    WHERE code_name = 'JAAPIVA'
      and code_header_id = 43;
    DELETE
    FROM invasivesbc.code
    WHERE code_name = 'BANGFAU'
      and code_header_id = 43;
  `);
