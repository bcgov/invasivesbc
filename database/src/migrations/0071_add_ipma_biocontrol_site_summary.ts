...
export async function up(knex: Knex): Promise<void> {
  await knex.raw(
    ...
    drop view iapp_site_summary_slow cascade;
    ...
    CREATE OR REPLACE VIEW invasivesbc.iapp_site_summary_slow
        AS WITH sites_grouped AS (
                ...
                (
               SELECT string_agg(DISTINCT biological_agent, ', ')
               FROM (
                   SELECT biological_agent FROM invasivesbc.biological_dispersal_extract WHERE site_id = sse_1.site_id
                   UNION
                   SELECT biological_agent FROM invasivesbc.biological_treatment_extract WHERE site_id = sse_1.site_id
                   UNION
                   SELECT biological_agent FROM invasivesbc.biological_monitoring_extract WHERE site_id = sse_1.site_id
               ) AS bio_agents
           ) AS biological_agent,
        ...
        SELECT sse.site_id,
          ...
          FROM sites_grouped sse
            ...
            rd_riso AS (SELECT distinct(site_id), regional_district, regional_invasive_species_organization, invasive_plant_management_area
            FROM invasivesbc.site_selection_extract s
            group by site_id, regional_district, regional_invasive_species_organization, invasive_plant_management_area
            )
              SELECT i.site_id,
             ...
             DROP MATERIALIZED VIEW IF EXISTS invasivesbc.iapp_site_summary_and_geojson;
              ...
              max_survey, agencies, has_biological_treatments, biological_agent, has_chemical_treatments,  
              has_mechanical_treatments, has_biological_dispersals, monitored, all_species_on_site_as_array,
              regional_district, regional_invasive_species_organization, invasive_plant_management_area);
