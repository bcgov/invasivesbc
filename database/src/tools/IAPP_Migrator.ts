          ...
          surveys: surveys.map((survey) => {
            ...
            if (speciesMatches[survey.Species]?.[survey.CommonName] !== undefined)
              ...
              Object.keys(invasiveSpeciesCodes).forEach((code) => {
                ...
                if (survey.CommonName && survey.CommonName.includes(fullname) && fullname.includes(survey.CommonName) && invasiveSpeciesCodes[code].includes(survey.Species))
                  matchedSpecies[code] = 'perfect';
                else if (survey.CommonName && invasiveSpeciesCodes[code].includes(survey.Species) && (
                ...
                else if (invasiveSpeciesCodes[code].includes(survey.Species) && survey.CommonName && invasiveSpeciesCodes[code].includes(survey.CommonName))
                  matchedSpecies[code] = 'good';
                else if (invasiveSpeciesCodes[code].includes(survey.Species))
                  matchedSpecies[code] = 'fair';
              ...
              switch (Object.keys(matchedSpecies).length) {
                case 0:
                  ...
                  break;
                case 1:
                  bestMatch = Object.keys(matchedSpecies)[0]; break;
                default:
                  const perfectMatches = Object.keys(matchedSpecies).filter((match) => matchedSpecies[match] === 'perfect');
                  ...
                  const excellentMatches = Object.keys(matchedSpecies).filter((match) => matchedSpecies[match] === 'excellent');
            ...
            if (bestMatch === undefined) {
              ...
              if (!speciesMatchFailures[survey.Species])
                speciesMatchFailures[survey.Species] = {};
              if (!speciesMatchFailures[survey.Species]?.[survey.CommonName]) {
                speciesMatchFailures[survey.Species][survey.CommonName] = 1;
                console.log('No match: ', speciesMatchFailures, speciesMatches, matchedSpecies, surveyMissedMatchCount, survey.Species, survey.CommonName);
              } else
                speciesMatchFailures[survey.Species][survey.CommonName] += 1;
            } else {
              if (bestMatch !== null) {
              ...
              if (!speciesMatches[survey.Species])
                speciesMatches[survey.Species] = {};
