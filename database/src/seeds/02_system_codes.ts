...
async function _load_category_and_header(knex: Knex): Promise<any> {
    ...
    const category_id = (await knex("code_category").withSchema('invasivesbc').returning('code_category_id').insert([
        { code_category_name: 'system', code_category_title: 'System Codes', 
            code_category_description: 'The category for system codes', created_by_user_id: 1, updated_by_user_id: 1},
    ]))[0];
...
async function _load_codes(knex: Knex, header_id: any): Promise<void> {
  ...
  await new Promise((resolve, reject) => {
    ...
    .on('data', (dataRow) => {
      const extendedRow = {...dataRow, code_header_id: header_id, valid_from: knex.fn.now(), created_by_user_id: 1, updated_by_user_id: 1}
