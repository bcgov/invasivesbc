...
from invasivesbc.db_models.codes import BiocontrolAgentCode, TerrestrialPlantCode
...
class TerrestrialBiocontrolRelease(BaseOneToManyActivityTable):
  ...
  plant_collected_from = models.ForeignKey(TerrestrialPlantCode, on_delete=models.PROTECT, blank=True, null=True, related_name="additional_plant_found_on")
  plant_collected_from_manual = models.CharField(max_length=128, blank=True, null=True)

  class Meta:
    # db_table='"activity"."terrestrial_biocontrol_dispersal_monitoring"'
    constraints = [
      models.UniqueConstraint(fields=["activity_id", "invasive_plant", "biocontrol_agent"], name="unique_biocontrol_release")
    ]

  def clean(self):
    super().clean()
    if self.collection_date > timezone.now():
      raise ValidationError({"start_time_collecting": "Start time for collection cannot occur in the future"})
