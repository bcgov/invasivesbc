...
import { AUTH_INITIALIZE_COMPLETE, MAP_TOGGLE_BASEMAP, MAP_TOGGLE_GEOJSON_CACHE, RECORDSET_REMOVE_FILTER, RECORDSET_UPDATE_FILTER, URL_CHANGE } from 'state/actions';
...
describe('Can trigger refetch for both table and data on filter change', function () {
  ...
  it('can deal with adding a spatial filter', async function () {
    let initialHash = store.getState()?.UserSettings?.recordSets?.[2].tableFiltersHash;
