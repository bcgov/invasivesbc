...
export function populateTransectLinesLengthAndBearing(newSubtypeData: any): any {
  ...
  const transectLinesMatchingKeys = Object.keys(updatedActivitySubtypeData).filter(key => key.includes('transect_lines'));
  ...
  */
  ...
  transectLinesList.forEach((transectLineObj: any) => {
    ...
    if (start_x_utm && end_x_utm && start_y_utm && end_y_utm) {
      transectLine.transect_bearing = (Math.atan((end_x_utm - start_x_utm) / (end_y_utm - start_y_utm)) * (180 / Math.PI)).toFixed(1);
