...
export const SortableChild = (props: any) => {
    ...
    return (
      <Grid id={child.id} container style={{ marginBottom: -5, marginTop: -5 }} direction="row" alignItems="center">
        ...
        <Grid item xs={1} alignContent="center" justifyContent="center">
          <Checkbox
            id="child-checkbox"
            checked={getChild(layers, props.parent.id, child.id).enabled}
            name={child.name}
            onChange={() => {
              updateChild(layers, setLayers, props.parent.id, child.id, {
                enabled: !getChild(layers, props.parent.id, child.id).enabled
              });
            }}
          />
        </Grid>
  ...
  const onSortEnd = ({ oldIndex, newIndex }: any) => {
    ...
    updateParent(layers, setLayers, props.parent.id, { children: sortArray(returnVal) });
  };
