...
/* HelperFiles */
import { sortArray, sortObject } from './SortLayerOrder';
...
export const SortableParent = (props: any) => {
  ...
  const onSortEnd = ({ oldIndex, newIndex }: any) => {
    const returnVal = sortObject(sortArray(layers), oldIndex, newIndex);
    setLayers(sortArray(returnVal));
  };

  return <SortableListContainer items={sortArray(layers)} onSortEnd={onSortEnd} useDragHandle={true} lockAxis="y" />;
};
