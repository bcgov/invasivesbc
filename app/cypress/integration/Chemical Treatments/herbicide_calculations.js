...
describe('can test for weed poison', function () {
  ...
  context('src/utils/herbicideCalculator.ts', function () {
    ...
    it('passed scenario #2 -- amounts_of_mix_used (specie1)', function () {
      expect(
        mSpecie_sLHerb_spray_usingProdAppRate(area, 2, 5, 400, [75, 25]).invasive_plants[0].amount_of_mix_used
      ).to.eq(3.75);
    });
    it('passed scenario #2 -- amounts_of_mix_used (specie2)', function () {
      expect(
        mSpecie_sLHerb_spray_usingProdAppRate(area, 2, 5, 400, [75, 25]).invasive_plants[1].amount_of_mix_used
      ).to.eq(1.25);
    });
    it('passed scenario #2 -- areas_treated_hectares (specie1)', function () {
      expect(
        mSpecie_sLHerb_spray_usingProdAppRate(area, 2, 5, 400, [75, 25]).invasive_plants[0].area_treated_hectares
      ).to.eq(0.0094);
    });
    it('passed scenario #2 -- areas_treated_hectares (specie2)', function () {
      expect(
        mSpecie_sLHerb_spray_usingProdAppRate(area, 2, 5, 400, [75, 25]).invasive_plants[1].area_treated_hectares
      ).to.eq(0.0031);
    });
    it('passed scenario #2 -- areas_treated_sqm (specie1)', function () {
      expect(
        mSpecie_sLHerb_spray_usingProdAppRate(area, 2, 5, 400, [75, 25]).invasive_plants[0].area_treated_sqm
      ).to.eq(93.75);
    });
    it('passed scenario #2 -- areas_treated_sqm (specie2)', function () {
      expect(
        mSpecie_sLHerb_spray_usingProdAppRate(area, 2, 5, 400, [75, 25]).invasive_plants[1].area_treated_sqm
      ).to.eq(31.25);
    });
    it('passed scenario #2 -- percentages_area_covered (specie1)', function () {
      expect(
        mSpecie_sLHerb_spray_usingProdAppRate(area, 2, 5, 400, [75, 25]).invasive_plants[0].percentage_area_covered
      ).to.eq(56.8182);
    });
    it('passed scenario #2 -- percentages_area_covered (specie2)', function () {
      expect(
        mSpecie_sLHerb_spray_usingProdAppRate(area, 2, 5, 400, [75, 25]).invasive_plants[1].percentage_area_covered
      ).to.eq(18.9394);
    });
    it('passed scenario #2 -- amounts_of_undiluted_herbicide_used (specie1)', function () {
      expect(
        mSpecie_sLHerb_spray_usingProdAppRate(area, 2, 5, 400, [75, 25]).invasive_plants[0]
          .amount_of_undiluted_herbicide_used_liters
      ).to.eq(0.0188);
    });
    it('passed scenario #2 -- amounts_of_undiluted_herbicide_used (specie2)', function () {
      expect(
        mSpecie_sLHerb_spray_usingProdAppRate(area, 2, 5, 400, [75, 25]).invasive_plants[1]
          .amount_of_undiluted_herbicide_used_liters
      ).to.eq(0.0063);
    });
    ...
    it('passed scenario #4 -- areas_treated_sqm (specie #1)', function () {
      expect(
        mSpecie_sLHerb_spray_usingDilutionPercent(area, 5, 2, 125, [75, 25]).invasive_plants[0].area_treated_sqm
      ).to.eq(93.75);
    });
    it('passed scenario #4 -- areas_treated_sqm (specie #2)', function () {
      expect(
        mSpecie_sLHerb_spray_usingDilutionPercent(area, 5, 2, 125, [75, 25]).invasive_plants[1].area_treated_sqm
      ).to.eq(31.25);
    });
    it('passed scenario #4 -- percentages_area_covered (specie #1)', function () {
      expect(
        mSpecie_sLHerb_spray_usingDilutionPercent(area, 5, 2, 125, [75, 25]).invasive_plants[0].percentage_area_covered
      ).to.eq(56.8182);
    });
    it('passed scenario #4 -- percentages_area_covered (specie #2)', function () {
      expect(
        mSpecie_sLHerb_spray_usingDilutionPercent(area, 5, 2, 125, [75, 25]).invasive_plants[1].percentage_area_covered
      ).to.eq(18.9394);
    });
    it('passed scenario #4 -- amounts_of_undiluted_herbicide_used (specie #1)', function () {
      expect(
        mSpecie_sLHerb_spray_usingDilutionPercent(area, 5, 2, 125, [75, 25]).invasive_plants[0]
          .amount_of_undiluted_herbicide_used_liters
      ).to.eq(0.075);
    });
    it('passed scenario #4 -- amounts_of_undiluted_herbicide_used (specie #2)', function () {
      expect(
        mSpecie_sLHerb_spray_usingDilutionPercent(area, 5, 2, 125, [75, 25]).invasive_plants[1]
          .amount_of_undiluted_herbicide_used_liters
      ).to.eq(0.025);
    });
    ...
    it('passed scenario #5 -- dilution', function () {
      expect(mSpecie_sGHerb_spray_usingProdAppRate(area, 230, 5, 400, [100]).dilution).to.eq(57.5);
    });
    it('passed scenario #5 -- area_treated_hectares', function () {
      expect(
        mSpecie_sGHerb_spray_usingProdAppRate(area, 230, 5, 400, [100]).invasive_plants[0].area_treated_hectares
      ).to.eq(0.0125);
    });
    it('passed scenario #5 -- area_treated_sqm', function () {
      expect(mSpecie_sGHerb_spray_usingProdAppRate(area, 230, 5, 400, [100]).invasive_plants[0].area_treated_sqm).to.eq(
        125
      );
    });
    it('passed scenario #5 -- percent_area_covered', function () {
      expect(
        mSpecie_sGHerb_spray_usingProdAppRate(area, 230, 5, 400, [100]).invasive_plants[0].percentage_area_covered
      ).to.eq(75.7576);
    });
    it('passed scenario #5 -- amount_of_mix_used', function () {
      expect(
        mSpecie_sGHerb_spray_usingProdAppRate(area, 230, 5, 400, [100]).invasive_plants[0].amount_of_mix_used
      ).to.eq(5);
    });
    it('passed scenario #5 -- amount_of_undiluted_herbicide_used_liters', function () {
      expect(
        mSpecie_sGHerb_spray_usingProdAppRate(area, 230, 5, 400, [100]).invasive_plants[0]
          .amount_of_undiluted_herbicide_used_liters
      ).to.eq(2.875);
    });
    ...
    it('passed scenario #6 -- area_treated_hectares', function () {
      expect(mSpecie_sGHerb_spray_usingDilutionPercent(area, 5, 0.023, 100, [100]).area_treated_hectares).to.eq(0.01);
    });
    it('passed scenario #6 -- percent_area_covered', function () {
      expect(
        mSpecie_sGHerb_spray_usingDilutionPercent(area, 5, 0.023, 100, [100]).invasive_plants[0].percentage_area_covered
      ).to.eq(60.6061);
    });
    it('passed scenario #6 -- amount_of_undiluted_herbicide_used_liters', function () {
      expect(
        mSpecie_sGHerb_spray_usingDilutionPercent(area, 5, 0.023, 100, [100]).invasive_plants[0]
          .amount_of_undiluted_herbicide_used_liters
      ).to.eq(0.0011);
    });
    it('passed scenario #6 -- area_treated_sqm', function () {
      expect(
        mSpecie_sGHerb_spray_usingDilutionPercent(area, 5, 0.023, 100, [100]).invasive_plants[0].area_treated_sqm
      ).to.eq(100);
    });
