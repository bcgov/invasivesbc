...
describe('can test for weed poison', function () {
  ...
  context('src/utils/herbicideCalculator.ts', function () {
    ...
    it('passed scenario #1 -- dilution', function () {
      expect((sSpecie_sLHerb_spray_usingProdAppRate(area, 2, 5, 400) as any).dilution).to.eq(0.5);
    });
    it('passed scenario #1 -- area_treated_hectares', function () {
      expect((sSpecie_sLHerb_spray_usingProdAppRate(area, 2, 5, 400) as any).area_treated_hectares).to.eq(0.0125);
    });
    it('passed scenario #1 -- area_treated_sqm', function () {
      expect((sSpecie_sLHerb_spray_usingProdAppRate(area, 2, 5, 400) as any).area_treated_sqm).to.eq(125);
    });
    it('passed scenario #1 -- percent_area_covered', function () {
      expect((sSpecie_sLHerb_spray_usingProdAppRate(area, 2, 5, 400) as any).percent_area_covered).to.eq(75.7576);
    });
    it('passed scenario #1 -- amount_of_undiluted_herbicide_used_liters', function () {
      expect(
        (sSpecie_sLHerb_spray_usingProdAppRate(area, 2, 5, 400) as any).amount_of_undiluted_herbicide_used_liters
      ).to.eq(0.025);
    });
    ...
    it('passed scenario #2 -- dilution', function () {
      expect((mSpecie_sLHerb_spray_usingProdAppRate(area, 2, 5, 400, [75, 25]) as any).dilution).to.eq(0.5);
    });
    it('passed scenario #2 -- amounts_of_mix_used (specie1)', function () {
      expect(
        (mSpecie_sLHerb_spray_usingProdAppRate(area, 2, 5, 400, [75, 25]) as any).invasive_plants[0].amount_of_mix_used
      ).to.eq(3.75);
    });
    it('passed scenario #2 -- amounts_of_mix_used (specie2)', function () {
      expect(
        (mSpecie_sLHerb_spray_usingProdAppRate(area, 2, 5, 400, [75, 25]) as any).invasive_plants[1].amount_of_mix_used
      ).to.eq(1.25);
    });
    it('passed scenario #2 -- areas_treated_hectares (specie1)', function () {
      expect(
        (mSpecie_sLHerb_spray_usingProdAppRate(area, 2, 5, 400, [75, 25]) as any).invasive_plants[0]
          .area_treated_hectares
      ).to.eq(0.0094);
    });
    it('passed scenario #2 -- areas_treated_hectares (specie2)', function () {
      expect(
        (mSpecie_sLHerb_spray_usingProdAppRate(area, 2, 5, 400, [75, 25]) as any).invasive_plants[1]
          .area_treated_hectares
      ).to.eq(0.0031);
    });
    it('passed scenario #2 -- areas_treated_sqm (specie1)', function () {
      expect(
        (mSpecie_sLHerb_spray_usingProdAppRate(area, 2, 5, 400, [75, 25]) as any).invasive_plants[0].area_treated_sqm
      ).to.eq(93.75);
    });
    it('passed scenario #2 -- areas_treated_sqm (specie2)', function () {
      expect(
        (mSpecie_sLHerb_spray_usingProdAppRate(area, 2, 5, 400, [75, 25]) as any).invasive_plants[1].area_treated_sqm
      ).to.eq(31.25);
    });
    ...
    it('passed scenario #3 -- area_treated_hectares', function () {
      expect((sSpecie_sLHerb_spray_usingDilutionPercent(area, 5, 2, 125) as any).area_treated_hectares).to.eq(0.0125);
    });
    it('passed scenario #3 -- percent_area_covered', function () {
      expect((sSpecie_sLHerb_spray_usingDilutionPercent(area, 5, 2, 125) as any).percent_area_covered).to.eq(75.7576);
    });
    it('passed scenario #3 -- amount_of_undiluted_herbicide_used_liters', function () {
      expect(
        (sSpecie_sLHerb_spray_usingDilutionPercent(area, 5, 2, 125) as any).amount_of_undiluted_herbicide_used_liters
      ).to.eq(0.1);
    });

    //SCENARIO #4 =============================================================================
    it('passed scenario #4 -- area_treated_hectares', function () {
      expect((mSpecie_sLHerb_spray_usingDilutionPercent(area, 5, 2, 125, [75, 25]) as any).area_treated_hectares).to.eq(
        0.0125
      );
    });
    it('passed scenario #4 -- areas_treated_sqm (specie #1)', function () {
      expect(
        (mSpecie_sLHerb_spray_usingDilutionPercent(area, 5, 2, 125, [75, 25]) as any).invasive_plants[0]
          .area_treated_sqm
      ).to.eq(93.75);
    });
    it('passed scenario #4 -- areas_treated_sqm (specie #2)', function () {
      expect(
        (mSpecie_sLHerb_spray_usingDilutionPercent(area, 5, 2, 125, [75, 25]) as any).invasive_plants[1]
          .area_treated_sqm
      ).to.eq(31.25);
    });
    ...
    it('passed scenario #5 -- dilution', function () {
      expect((mSpecie_sGHerb_spray_usingProdAppRate(area, 230, 5, 400, [100]) as any).dilution).to.eq(57.5);
    });
    it('passed scenario #5 -- area_treated_hectares', function () {
      expect(
        (mSpecie_sGHerb_spray_usingProdAppRate(area, 230, 5, 400, [100]) as any).invasive_plants[0]
          .area_treated_hectares
      ).to.eq(0.0125);
    });
    it('passed scenario #5 -- area_treated_sqm', function () {
      expect(
        (mSpecie_sGHerb_spray_usingProdAppRate(area, 230, 5, 400, [100]) as any).invasive_plants[0].area_treated_sqm
      ).to.eq(125);
    });
    ...
    it('passed scenario #5 -- amount_of_mix_used', function () {
      expect(
        (mSpecie_sGHerb_spray_usingProdAppRate(area, 230, 5, 400, [100]) as any).invasive_plants[0].amount_of_mix_used
      ).to.eq(5);
    });
    ...
    it('passed scenario #6 -- area_treated_hectares', function () {
      expect(
        (mSpecie_sGHerb_spray_usingDilutionPercent(area, 5, 0.023, 100, [100]) as any).area_treated_hectares
      ).to.eq(0.01);
    });
    it('passed scenario #6 -- percent_area_covered', function () {
      expect(
        (mSpecie_sGHerb_spray_usingDilutionPercent(area, 5, 0.023, 100, [100]) as any).invasive_plants[0]
          .percentage_area_covered
      ).to.eq(60.6061);
    });
    ...
    it('passed scenario #6 -- area_treated_sqm', function () {
      expect(
        (mSpecie_sGHerb_spray_usingDilutionPercent(area, 5, 0.023, 100, [100]) as any).invasive_plants[0]
          .area_treated_sqm
      ).to.eq(100);
    ...
    it('passed scenario #7 -- area_treated_hectares', function () {
      expect((sSpecie_sLHerb_direct_usingDilutionPercent(area, 1, 50, 30) as any).area_treated_hectares).to.eq(0.003);
    });

    it('passed scenario #7 -- percent_area_covered', function () {
      expect((sSpecie_sLHerb_direct_usingDilutionPercent(area, 1, 50, 30) as any).percent_area_covered).to.eq(18.1818);
    });

    it('passed scenario #7 -- amount_of_undiluted_herbicide_used_liters', function () {
      expect(
        (sSpecie_sLHerb_direct_usingDilutionPercent(area, 1, 50, 30) as any).amount_of_undiluted_herbicide_used_liters
      ).to.eq(0.5);
    });
    ...
    it('passed scenario #8', function () {
      ...
      expect(
        mSpecie_mLGHerb_spray_usingProdAppRate(area, 5, 400, speciesArr, herbicideArr)[0].amount_of_mix_used
      ).to.eq(3.75);
      expect(
        (mSpecie_mLGHerb_spray_usingProdAppRate(area, 5, 400, speciesArr)[0] as any).herbicides[0]
          .product_application_rate_lha
      ).to.eq(2);
      expect(
        mSpecie_mLGHerb_spray_usingProdAppRate(area, 5, 400, speciesArr, herbicideArr)[0].herbicides[0].dilution
      ).to.eq(0.5);
      expect(
      ...
      expect(
        mSpecie_mLGHerb_spray_usingProdAppRate(area, 5, 400, speciesArr, herbicideArr)[0].herbicides[1].dilution
      ).to.eq(0.16);
      expect(
      ...
      expect(mSpecie_mLGHerb_spray_usingProdAppRate(area, 5, 400, speciesArr, herbicideArr)[1].area_of_specie).to.eq(
        25
      );
      expect(mSpecie_mLGHerb_spray_usingProdAppRate(area, 5, 400, speciesArr, herbicideArr)[1].area_treated_ha).to.eq(
        0.0031
      );
      expect(mSpecie_mLGHerb_spray_usingProdAppRate(area, 5, 400, speciesArr, herbicideArr)[1].area_treated_sqm).to.eq(
        31.25
      );
      expect(
        mSpecie_mLGHerb_spray_usingProdAppRate(area, 5, 400, speciesArr, herbicideArr)[1].percent_area_covered
      ).to.eq(18.9394);
      expect(
        mSpecie_mLGHerb_spray_usingProdAppRate(area, 5, 400, speciesArr, herbicideArr)[1].amount_of_mix_used
      ).to.eq(1.25);
      expect(
        mSpecie_mLGHerb_spray_usingProdAppRate(area, 5, 400, speciesArr)[1].herbicides[0].product_application_rate_lha
      ).to.eq(2);
      expect(
        mSpecie_mLGHerb_spray_usingProdAppRate(area, 5, 400, speciesArr, herbicideArr)[1].herbicides[0].dilution
      ).to.eq(0.5);
      expect(
