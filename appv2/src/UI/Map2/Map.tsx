...
import {
  ...
  refreshClientBoundariesOnToggle, checkPublicMap
} from './Helpers';
...
export const Map = (props: any) => {
    ...
    const [styleLoadComplete, setStyleLoadComplete] = useState(false);
    const [styleLoadTTL, setStyleLoadTTL] = useState(500);

    useEffect(() => {
      if (!map.current) {
        setStyleLoadComplete(false);
        if (styleLoadTTL > 0) {
          const tid = setTimeout(() => {
            setStyleLoadTTL(styleLoadTTL - 1);
          }, 100);
          return () => {
            clearTimeout(tid);
          };
...
// Public Layers
    ...
    const publicLayersHash = useSelector((state: any) => state.Map.publicLayersHash);
...
// RecordSet Layers:
    ...
    useEffect(() => {
      if (!map.current) return;
      if (!styleLoadComplete) return;
      checkPublicMap(publicLayers, map.current, API_BASE);
    }, [publicLayers, publicLayersHash, styleLoadComplete]);
