...
class MechanicalTreatmentMonitoringTest(TestCase):
  ...
  def test_no_pac_number_present(self):
    ...
    try:
      ...
      self.assertIsNotNone(serial.data["subtype_data"]["treatment_monitoring_information"])
  ...
  def test_casting_fixture_into_serializer(self):
    ...
    try:
      record = ActivityBasic.objects.get(activity_id="CD542709-F767-402F-818E-117B3FBC797D")
      self.assertIsNotNone(record)

      serial = ActivitySerializer(record)
      # Check a subtype specific field
      self.assertEqual(len(serial.data["subtype_data"]["treatment_monitoring_information"]), 2)
    except ActivityBasic.DoesNotExist:
      self.fail("Activity does not exist")
