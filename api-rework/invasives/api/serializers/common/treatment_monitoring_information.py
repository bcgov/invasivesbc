...
class TerrestrialTreatmentMonitoringSerializer(BaseTreatmentMonitoringSerializer):
  ...
  def get_invasive_plants_on_site(self, obj):
    ...
    try:
      invasive_plants_on_site = TerrestrialInvasivePlantOnSite.objects.filter(activity_id=activity_id, invasive_plant=invasive_plant)
      return TerrestrialInvasivePlantOnSiteSerializer(invasive_plants_on_site, many=True).data
...
class AquaticMechanicalMonitoringSerializer(BaseTreatmentMonitoringSerializer):
  ...
  def get_invasive_plants_on_site(self, obj):
    ...
    invasive_plants_on_site = AquaticInvasivePlantOnSite.objects.filter(activity_id=activity_id, invasive_plant=invasive_plant)
    return AquaticInvasivePlantOnSiteSerializer(invasive_plants_on_site, many=True).data
...
class TreatmentMonitoringInfoSerializer(serializers.Serializer):
  ...
  a_monitoring_information = AquaticMechanicalMonitoringSerializer(source="aquatictreatmentmonitoringinformation_set", many=True, required=False)
  t_monitoring_information = TerrestrialTreatmentMonitoringSerializer(source="terrestrialtreatmentmonitoringinformation_set", many=True, required=False)
