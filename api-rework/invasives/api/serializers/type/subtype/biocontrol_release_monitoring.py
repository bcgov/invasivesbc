...
class BiocontrolReleaseMonitoringSerializer(BaseSerializer):
...
class DraftBiocontrolReleaseMonitoringSerializer(BaseSerializer):

    def get_spread_results(self, obj):
        children = DraftSpreadResults.objects.filter(
            activity_data_record__activity_id=obj.id
        ).first()
        return (
            DraftSpreadResultsSerializer(children).data
            if children is not None
            else None
        )
