    ...
    for (
        ...
        m = monitoring_entry_class.objects.create(
            ...
            management_efficacy_rating=(
                EfficacyManagementRatingCode.objects.get(
                    code=mt.management_efficacy_rating
                )
        ...
        known_good_monitoring_codes = ["FSUP", "LAU", "NSR", "NA", "ROP", "SWS", "SWOS"]

        for pos in mt.invasive_plants_on_site.split(","):
            if pos not in known_good_monitoring_codes:
                raise ValueError(f"unknown monitoring code {pos}")

        m.monitoring_evidence_codes = mt.invasive_plants_on_site
