...
class RegionalDetail(BaseOneToOneActivityTable):
  ...
  elevation_m = models.SmallIntegerField(blank=True, db_comment="Estimated elevation of an area in meters")
  ...
  def clean(self):
    super().clean()
    if not self.biogeoclimatic_zone and \
       not self.invasive_plant_management_areas and \
       not self.ownership and \
       not self.regional_districts and \
       not self.flrno_districts and \
       not self.moti_districts and \
       not self.elevation_m:
      raise ValidationError(_("At least one value other than activity_id must be non-null"), code="invalid")
