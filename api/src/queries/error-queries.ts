...
export const saveErrorSQL = (searchCriteria: ErrorPostRequestBody): SQLStatement => {
  ...
  if (searchCriteria.error) {
    sqlStatement.append(SQL`
      insert into invasivesbc.error_log (error, client_state, created_by, created_by_with_guid)
      values (${searchCriteria.error}, ${searchCriteria.clientState}, ${searchCriteria.created_by},
              ${searchCriteria.created_by_with_guid});
    `);
  }
