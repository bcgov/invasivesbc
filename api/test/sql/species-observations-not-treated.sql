...
select
  ...
  case -- Geometry conditional subtractions
    when public.st_intersects(p1.geom,p2.geom) -- If treatment overlaps observation
    then public.st_difference(p1.geom,p2.geom) -- Subtract the treatment area from observations
...
from
  ...
  **/
  ( -- Select treatments
    select
      i1.species "species", 
      public.st_union(i1.geom) "geom", -- Merge treatments touching the same observations
    ...
    where
      ...
      public.st_intersects(i2.geom,i1.geom) and -- Where treatments touch an observations
      i1.species = i2.species
    group by
      i1.species
  ) p2
  on
    public.st_intersects(p2.geom,p1.geom) and -- Where treatments touch observations
